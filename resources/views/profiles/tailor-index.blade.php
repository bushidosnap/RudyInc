    ...
    <div class="row bg-secondary rounded mt-4 pb-5">
        <div class="col-sm-12">
            <div class="d-flex align-items-baseline">
                <div class="row pt-2 pl-2 text-white mr-auto p-2"><h1>Tailor's Dashboard</h1></div>
                <div class="p-2">
                    <a href={{route('tailor-create') }} class="btn btn-primary">Create</a>
                    <a href="#" class="btn btn-warning" aria-pressed="true">Update</a>
                </div>
                <div class="p-2">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table table-bordered">
                        ...
                        @foreach ($tailorShow as $tailorShow)
                            <tr>
                                <td>{{$tailorShow->id}}</td>
                                <td>
                                    <button type = "button" class = "btn btn-primary btn_showItem" id = "{{$tailorShow->id}}">
                                        View Item/(s)
                                     </button>
                                     
                                     <div id = "demo{{$tailorShow->id}}" class = "collapse in">
                                        
                                     </div>      
                                </td>
                                <td>{{$tailorShow->client_email}}</td>
                                <td>{{\App\Helpers\Helper::showDateTime($tailorShow->date_finish)}}</td>
                                @if($tailorShow->status == 1)
                                <td><button class="btn btn-secondary" disabled>{{\App\Helpers\Helper::showOrderStatus($tailorShow->status)}}</button></td>
                                @else
                                <td><a href="#" 
                                    @if($tailorShow->status == 1)
                                    class="btn btn-secondary btn_changeStatus"
                                    @elseif($tailorShow->status == 2)
                                    class="btn btn-warning btn_changeStatus"
                                    @elseif($tailorShow->status == 3)
                                    class="btn btn-success btn_changeStatus"
                                    @elseif($tailorShow->status == 4)
                                    class="btn btn-primary btn_changeStatus"
                                    @endif
                                    id="{{$tailorShow->id}}">{{\App\Helpers\Helper::showOrderStatus($tailorShow->status)}}</a></td>
                                @endif
